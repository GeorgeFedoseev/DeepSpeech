...
def create_lm(vocabulary_path, o=5, prune=2):
    ...
    lmplz_path = os.path.join(const.DEEP_SPEECH_LM_DIR_PATH, "kenlm/build/bin/lmplz")
    build_binary_path = os.path.join(const.DEEP_SPEECH_LM_DIR_PATH, "kenlm/build/bin/build_binary")
    ...
    p = subprocess.Popen([
        "%s --text %s --arpa %s --o %i --prune %s -S 50%" % (lmplz_path, vocabulary_path, const.DEEP_SPEECH_ARPA_PATH, o, prune_str)
        + " & %s -T -s %s %s" % (build_binary_path, const.DEEP_SPEECH_ARPA_PATH, const.DEEP_SPEECH_LM_PATH)
        + " & %s %s %s %s" % (generate_trie_path, const.DEEP_SPEECH_ALPHABET_PATH, const.DEEP_SPEECH_LM_PATH, vocabulary_path, const.DEEP_SPEECH_TRIE_PATH)
